...
def is_square():
    ...
    width = input('Enter the width of an area in inches:')
    height = input('Enter the height of an area in inches:')
    if width == height:
        return True
    else:
        return False
...
def get_discount():
    ...
    """
    #### write your solution for this function below here. ####
    masks = int(input('How many masks you would like to have?'))
    cost = masks * 5
    if masks >= 5000:
        cost = cost * 0.8
    return f"${cost:,.0f}"
